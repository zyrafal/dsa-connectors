...
describe("B.Maker", function () {
  ...
  before(async () => {
    ...
    manager = await ethers.getContractAt(
      "BManagerLike",
      "0x3f30c2381CD8B917Dd96EB2f1A4F96D91324BBed"
    );
    vat = await ethers.getContractAt(
      "../artifacts/contracts/mainnet/connectors/b.protocol/makerdao/interface.sol:VatLike",
      await manager.vat()
    );
    dai = await ethers.getContractAt(
      "../artifacts/contracts/mainnet/common/interfaces.sol:TokenInterface",
      tokens.dai.address
    );
  ...
  describe("Main", function () {
    ...
    it("Should repay", async function () {
      ...
      const urnData = await vat.urns(ilk, urn);
      expect(urnData[0]).to.be.equal(ethers.utils.parseEther("6")); // ink
      expect(urnData[1]).to.be.equal(
        await daiToArt(vat, ilk, ethers.utils.parseEther("5500"))
      ); // art
      expect(await dai.balanceOf(dsaWallet0.address)).to.be.equal(
    ...
    it("Should depositAndBorrow", async function () {
      ...
      expect(await dai.balanceOf(dsaWallet0.address)).to.be.equal(
        ethers.utils.parseEther("6500")
      );
      // calculation is not precise as the jug was dripped
      expect(
        veryClose(
          urnData[1],
          await daiToArt(vat, ilk, ethers.utils.parseEther("6500"))
        )
      ).to.be.true;
      expect(await ethers.provider.getBalance(dsaWallet0.address)).to.be.gte(
